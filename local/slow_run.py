#!/usr/bin/env python3
# Slower-verify-ramp test, 2026-09-23, smooth PEI, 112.7Hz. Run on zer0 (python 3.7).
...
for i, (x, y) in enumerate(POINTS):
    ...
    for arm in order:
        before = verifies()
        r = gcode("RESONANCE_PROBE_CONTACT POINT=%d,%d,2 FREQ=112.7 ACCEL_AXIS=x SPEED=0.2 SAMPLES=7 VERIFY_REPS=2"
                  " VERIFY_UP=0.14 VERIFY_DOWN=0.10 VERIFY_RAMP_SPEED=%.2f VERBOSE=1" % (x, y, ARMS[arm]))
